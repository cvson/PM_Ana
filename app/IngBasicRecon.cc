...
int main(int argc,char *argv[]){
  ...
  bool cosmic=false;
  while ((c = getopt(argc, argv, "r:s:f:o:c")) != -1) {
    switch(c){
    case 'r':
  ...
  /* by kikawa
  sprintf(FileName,"%s/ingrid_%08d_%04d_tclster.root",
  ...
  */
  ...
  if(!rename){
    sprintf(Output,"%s/ingrid_%08d_%04d_recon2.root",
	    dst_data, run_number, sub_run_number);
  }
  if(cosmic){
    sprintf(Output,"%s/ingrid_%08d_%04d_recon2.root",
