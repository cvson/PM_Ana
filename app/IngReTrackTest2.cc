...
int main(int argc,char *argv[]){
  ...
  while ((c = getopt(argc, argv, "r:s:f:b:i:e:")) != -1) {
    switch(c){
    case 'i':
      TestAreaStartPln=atoi(optarg);
      break;
    case 'e':
      TestAreaEndPln=atoi(optarg);
      break;
  ...
  //######## recreated root file ########################
  //#####################################################
  sprintf( Output, "%s.retrack_%d_%d.root", FileName, TestAreaStartPln, TestAreaEndPln  );
  //sprintf( Output, "t.root" );
  ...
  //for(int ievt=0; ievt<100000; ievt++){
    ...
    */
	...
	//#################################
	basicrecon->retracktest=false;
	if( retracking ){
	  basicrecon->retracktest=true;
	  //ntrkok++;
	  bool display = true;
#ifdef DEBUG
	  ...
	  if( frecontrack -> ReconTrack() ){
		...
		fabs( ExpEndY    - ReTrkEndY   ) < MatchDiff 
	      ...
	      //cout << " ****** OK *******" << endl;
	      basicrecon -> retracky = true;
	      display = false;
	    }
	    else{


	      basicrecon -> retracky = false;
	    }
	    /*
	    else{
	      cout << " ****** NO *******" << endl;
...
#ifdef DEBUG
	  if(!retracking)continue;
	  evtdisp     -> Update();
	if(basicrecon->retrackx)
	  cout << "X Success!" << endl;
	else
 	  cout << "X Fail!" << endl;
	if(basicrecon->retracky)
	  cout << "Y Success!" << endl;
	else
 	  cout << "Y Fail!" << endl;
  ...
  cout<<nretrkok0<<"/"<<ntrkok<<endl;
  cout<<nretrkok1<<"/"<<ntrkok<<endl;
}
