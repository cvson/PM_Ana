...
void  fill_beam(int utime){
  ...
  iMUMON[y][m][d]   += beaminfo->mumon[0+6];
  MUX [y][m][d]     += beaminfo->mumon[2+6];
  MUY [y][m][d]     += beaminfo->mumon[4+6];
  MUXxPOT [y][m][d] += beaminfo->mumon[2+6] * beaminfo->ct_np[4][0];
  MUYxPOT [y][m][d] += beaminfo->mumon[4+6] * beaminfo->ct_np[4][0];
}
