...
void Print(){
  ...
  if( fMeanPe>6.5 && nActPln>=2 && ontime && wohorn){//&& 
    //bunch_flag  && horn250 && nSpill==53306 ){
    if(UVETOHit){
    cout  << run_number << "\t"
	 << nSpill     << "\t"
	 << n          << "\t"
...
int main(int argc,char *argv[]){
  ...
  ModTree -> Branch("spill_flag"  ,&spill_flag,"spill_flag/B"        );
  ModTree -> Branch("bunch_flag"  ,&bunch_flag,"bunch_flag/B"        );
  ModTree -> Branch("tracking_posz"    ,&tracking_posz  ,"tracking_posz/D"       );
  ModTree -> Branch("horn250"     ,&horn250   ,"horn250/B"       );
  ModTree -> Branch("nurun"       ,&nurun     ,"nurun/I"       );
  ...
  fHbeam   -> Write();
  TH1F* test100126 = new TH1F("test100126","test100126",
			      11,0,11);
  ...
  for(n=0; n<Evt; n++){
    ...
    //if(bunch_flag||bflag){//#### Change at 10/01/25
    if(spill_flag||bflag){
    ...
    while(fFindTimeClster(allhit, hitclster, fTime, fcTime, dimch)){
      while( fTime  > 1000000000 ) fTime  -= 1000000000;
      //if( fTime  > 1000000000 ) fTime  -= 1000000000;
      while( fcTime > 1000000000 ) fcTime -= 1000000000;
      //if( fcTime > 1000000000 ) fcTime -= 1000000000;
	...
	MUTPLHit  = true;
      ...
      //##### Fill #######
      if( ct_np[0][ Cycle-4+1 ] > 1e11 && bunch_flag ) {
	...
	CTtime = 1.08e-6;
      ...
      //if( fabs( CTtime - 1.08e-6 ) > 0.3e-6 ){ //### Add at 
      //CTtime = 1.08e-6;
      //}
