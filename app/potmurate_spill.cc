...
ofstream outfile("murate.txt");

#define POTBIN 700
#define POTMAX 1.4e14

double npot[POTBIN],nmu[POTBIN];
int inttmp;
...
void  fill_beam(){
  double pot,mumon;
    pot  =beaminfo->ct_np[4][0];
    mumon=(beaminfo->mumon[0]);
    hmu ->Fill(pot,mumon);
    hpot->Fill(pot,pot);
    inttmp=(int)pot/POTMAX*POTBIN;
    npot[inttmp]+=pot;
    nmu[inttmp]+=mumon;
...
void  fill_ing(){
  ...
  if(ingbasic->hastrk && ingbasic->matchtrk && 
    ...
    if(mod<14&&cyc>=4&&cyc<12){
      hnu->Fill(pot[cyc-4]);
      inttmp=(int)pot[cyc-4]/POTMAX*POTBIN;
      nnu[mod][inttmp]++;
    }
