...
int main(int argc,char *argv[]){
  ...
  sprintf(buff1,"/home/daq/data/MPPC_calib/ingrid_00004109_0000.txt");
  ifstream file(buff1);
  Double_t pedestal[NumMod][2][NumTFB][NumCh];
  Double_t gain[NumMod][2][NumTFB][NumCh];
  int m,v,p,cc;
  double ped,gai;

  TH1F *fHg = new TH1F("fHg","fHg",100,5,15);
  TCanvas*  cx         = new TCanvas("cx","cx",10,10,700,700);
  TCanvas*  cy         = new TCanvas("cy","cy",10,10,700,700);

  while(file>>m>>v>>p>>cc>>ped>>gai){
    pedestal[m][v][p][cc]=ped;
    gain[m][v][p][cc]=gai;
    if(fabs(gai-10)>4.5)cout<<m<<" "<<v<<" "<<p<<" "<<cc<<" error"<<endl;
    fHg->Fill(gai);
  }
  //cx->cd();
  //fHg->Draw();
  ...
  TH1F *fHp = new TH1F("fHp","fHp",100,0,10);
  TH1F *fHpy = new TH1F("fHpy","fHpy",100,0,10);
  Int_t Cycle;
  ...
  for(int n=0;n<Nevent;n++){
    ...
    //for(Int_t nummod =2; nummod<3; nummod++){if(Cycle==0){
      if(nummod==anaMod){//if(Cycle==anaCycle){//{if(abs(99992-n)<1000){
      ...
      ////////////////////////////activity/////////////////////////////////
      Bool_t HitPln[15];
      for(int i=0;i<15;i++)HitPln[i]=false;
      for(Int_t i=0;i<nhitx;i++){
	IngHit *fHitx  = (IngHit*)(arrx->At(i));
	Long_t timex   = fHitx->Time;
	Int_t plnx     = fHitx->Pln;
	Int_t chx      = fHitx->Ch;
	Int_t adcx     = fHitx->HighAdc;
	Double_t pex   = (1.0*adcx-pedestal[nummod][0][plnx][chx])/gain[nummod][0][plnx][chx];
	fHp->Fill(pex);
	for(Int_t j=0;j<nhity;j++){
	  IngHit *fHity  = (IngHit*)(arry->At(j));
	  Long_t timey   = fHity->Time;
	  Int_t plny     = fHity->Pln;
	  Int_t chy      = fHity->Ch;
	  Int_t adcy     = fHity->HighAdc;
	  Double_t pey   = (1.0*adcy-pedestal[nummod][1][plny][chy])/gain[nummod][1][plny][chy];
	  fHpy->Fill(pey);
	  if(fabs(timex-timey)<TdcRes&&
	     plnx==plny){//timing
	    // HitPln[plnx]=true;    
	    if(pey>6.5&&pex>6.5)HitPln[plnx]=true;     
	  }
      ...
      Int_t  nHitPln=0;
      for(Int_t i=0;i<11;i++)if(HitPln[i]){
	nHitPln++;
      }
      
      h->Fill(nHitPln);
      //cout<<"activity:"<<nHitPln<<endl;
  ...
  */
  ...
  cy->Update();
  for(int k=0;k<11;k++){
    cout<<"hit"<<k<<" "<<h->GetBinContent(k+1)<<endl;
  }
  cin.get();
