...
int main(int argc,char *argv[]){
  ...
  for( int ievt = 0; ievt < nevt; ievt++  ){
    ...
    wsummary -> Clear();
    tree -> GetEntry(ievt);
    char *ver = "update tracking 2011/12/07";
    wsummary -> run        =  evt -> run;
    wsummary -> event      =  evt -> event;
    wsummary -> runmode    =  evt -> runmode;
    wsummary -> trgid      =  evt -> trgid;
    wsummary -> version    =  *ver;
    wsummary -> date       =  evt -> date;
    wsummary -> time       =  evt -> time;
    wsummary -> trgtime    =  evt -> trgtime;
    wsummary -> nd280nspill=  evt -> nd280nspill;

    for(int imod=0; imod<nMod; imod++){
      for(int icyc=0; icyc<23; icyc++){
	int nhit = evt -> NIngridModHits(imod, icyc);
	for(int ihit = 0; ihit < nhit; ihit++){
	  inghitsum = (IngridHitSummary*) ( evt -> GetIngridModHit(ihit, imod, icyc) );
	  wsummary -> AddIngridModHit(inghitsum, imod, icyc);
	}//ihit
      }//icyc
    }//imod
