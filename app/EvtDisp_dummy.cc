...
#include "TApplication.h"
#include "PMrecon_dummy.hxx"
#include "PMdisp_dummy.h"
...
int main(int argc,char *argv[]){
  ...
  //sprintf(buff1,"/home/kikawa/scraid1/data/ingrid/ingrid_00013159_0012_done.root");
  sprintf(buff1,"/home/kikawa/ingrid_00013159_0012_done.root");
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	while(fFindTimeClster(allhit, hitcls, fcTime)){
	  ...
	  if(mod<14){
	    cout << "**************************************" <<endl;
	    time(&timer);
	    cout << "JST        : "<<ctime(&timer)
	         << "Event#     : " << jndex<<endl;
	    jndex++;
	    cout << "Module     : " << mod2<<endl;
	    cout << "Cycle      : " << cyc<<endl;
	    cout << "# of hits  : " <<ninghit<<endl;

	    int iv=0,ih=0;
	    bool vetod=false,fv=true;
	    for(int i=0;i<alltrack.size();i++){
	      if(alltrack[i].view==0)ih++;else iv++;
	      if(alltrack[i].veto)vetod=true;
	      if(alltrack[i].edge)fv=false;
	    }
	    if(iv<ih)ih=iv;
	    cout << "# of tracks: " <<ih<<endl;
	    ...
	    for(int i=0;i<alltrack.size();i++){
		  ...
		  <<" Edge:"<<alltrack[i].edge
		  <<" FC:"<<alltrack[i].stop<<endl;
	    }
	    */
