...
int main(int argc,char *argv[]){
  ...
  wtree->Branch("hit_scibar",&hit_scibar,"hit_scibar/O");
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
    ...
    bool ingridtrk[23];
    memset(ingridtrk,false,sizeof(ingridtrk));
    for( int mod=3; mod<17; mod++ ){   //### Module Loop
      if(mod!=3&&mod!=16)continue;
	...
	while(fFindTimeClster(allhit, hitcls, fcTime)){
	  ...
	  if(!(alltrack[0].ipln==alltrack[1].ipln)&&
	     !((alltrack[0].ipln==alltrack[1].ipln+1))&&
	     !((alltrack[1].ipln==alltrack[0].ipln+1))
	     )continue;

	  
	  if(mod!=16&&!alltrack[0].veto&&!alltrack[1].veto)continue;
	  if(mod!=16&&alltrack[0].ipln!=0&&!alltrack[1].ipln!=0)continue;
	  if(mod==16&&alltrack[0].stop&&alltrack[1].stop)continue;
	  if(mod==16&&(alltrack[0].veto||alltrack[1].veto))continue;
	  if(mod==16&&(alltrack[0].edge||alltrack[1].edge))continue;
	  ...
	  if(mod==16&&!ingridtrk[cyc])continue;
	  ...
	  for(int i=0;i<alltrack.size();i++){
	    ...
	    */
	    ...
	    bool apln[18];
	    int stype[2][18];
	    float ape[18],alope[18],acorr[18];
	    memset(apln,false,sizeof(apln));
	    memset(ape,0,sizeof(ape));
	    //memset(alope,0,sizeof(alope));
	    //memset(acorr,0,sizeof(acorr));
	    memset(stype,0,sizeof(stype));
	    for(TMP=0;TMP<alltrack[i].hit_n;TMP++){
	      int k;
	      if(i==0)k=1;else k=0;
	      float corr2=exp(-(1200-(alltrack[k].intcpt+alltrack[k].slope*zposi(mod,alltrack[i].view,alltrack[i].hit_pln[TMP])))/2417);

	      apln[alltrack[i].hit_pln[TMP]]=true;
	      //acorr[alltrack[i].hit_pln[TMP]]=corr/corr2;
	      if(alltrack[i].hit_pln[TMP]>0&&alltrack[i].hit_ch[TMP]>=8&&alltrack[i].hit_ch[TMP]<24){
		stype[0][alltrack[i].hit_pln[TMP]]++;
		//ape[alltrack[i].hit_pln[TMP]]+=alltrack[i].hit_pe[TMP]/2;
		//alope[alltrack[i].hit_pln[TMP]]+=alltrack[i].hit_lope[TMP]/2;
		ape[alltrack[i].hit_pln[TMP]]+=PE(alltrack[i].hit_pe[TMP],alltrack[i].hit_lope[TMP])*corr/corr2/2;
	      }
	      else{
		stype[1][alltrack[i].hit_pln[TMP]]++;
		//ape[alltrack[i].hit_pln[TMP]]+=alltrack[i].hit_pe[TMP];
		//alope[alltrack[i].hit_pln[TMP]]+=alltrack[i].hit_lope[TMP];
		ape[alltrack[i].hit_pln[TMP]]+=PE(alltrack[i].hit_pe[TMP],alltrack[i].hit_lope[TMP])*corr/corr2;
	      }

	      //hit_corr=corr/corr2;
	      //hit_pln=alltrack[i].hit_pln[TMP];
	      //hit_ch=alltrack[i].hit_ch[TMP];
	      //hit_pe=alltrack[i].hit_pe[TMP];
	      //hit_lope=alltrack[i].hit_lope[TMP];
	      //wtree->Fill();
	    }
	    bool firstpln=true;
	    bool secondpln=true;
	    for(TMP=0;TMP<18;TMP++){
	      if(!apln[TMP])continue;
	      /*
	      if(firstpln){
		firstpln=false;
		continue;
		...
		continue;
	      ...
	      */
	      ...
	      //hit_lope=alope[TMP];
	      //hit_corr=acorr[TMP];
	      if(stype[1][TMP]<=stype[0][TMP])hit_scibar=true;
	      else hit_scibar=false;
	      wtree->Fill();
	    }
	  }
