...
int check(){
  ...
  for(int mod=0;mod<14;mod++){
    //if( neut[1][mod]&&!neut[2][mod]&&!neut[0][mod])res=mod;
    //if(!neut[1][mod]&& neut[2][mod]&&!neut[0][mod])res=mod;

    //if(neut[1][mod]==1&& neut[2][mod]==0&&neut[0][mod]==0)res=mod;
    if((neut[1][mod]+neut[2][mod])!=neut[0][mod])res=mod;
...
int main(int argc,char *argv[]){
  ...
  for(int ievt=0; ievt<nevt; ievt++){
    ...
    int res=check();
    if(res!=-1)cout<<"Module:"<<res<<" Event#:"<<ievt<<" Raw1:"<<neut[1][res]<<" Raw2:"<<neut[2][res]<<" Pileup:"<<neut[0][res]<<endl;
  }//ievt
  //out();
