...
int main(int argc,char *argv[]){
  ...
  //sprintf(FileName,"/home/ingrid/data/inter_spill_cosmic/cosmic_ana/ingrid_%08d_%04d_tclster.root",
  //sprintf(FileName,"/home/ingrid/data/beam_ana/ingrid_%08d_%04d_trk.root",
  sprintf(FileName,"/home/ingrid/data/dst/ingrid_%08d_%04d_track.root",
	  run_number,
	  sub_run_number);

  sprintf(Output,"/home/ingrid/data/inter_spill_cosmic/rmu/ingrid_%08d_%04d_cosmicana.root",
	  run_number,
	  sub_run_number);
  ...
  */
  ...
  //for(int ievt=0; ievt<5000; ievt++){
	...
	for(int ihit=0;ihit<evt->NIngridModHits(imod,icyc); ihit++){
	  ...
	  inghitsum->gocosmic=false;
	  inghitsum->hitcosmic=false;
	}
      }
    }
    ...
    for(int ibrecon=0; ibrecon<nbrecon; ibrecon++){
      ...
      if( !(basicrecon->vetowtracking) )continue;
