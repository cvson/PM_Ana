...
int main(int argc,char *argv[]){
  ...
  if(!rename)
    sprintf(FileName,"/home/t2kingrid/data/dst/ingrid_%08d_%04d_done.root",
  ...
  if(!rename)
    sprintf(Output,"/home/t2kingrid/data/dst/ingrid_%08d_%04d_onlybasic.root",
	    run_number, sub_run_number);
  ...
  for( int ievt = 0; ievt < nevt; ievt++  ){
    ...
    */
    for( int ibrec=0; ibrec < evt -> NPMAnas(); ibrec++ ){
      PMAnaSummary* pmanasum = (PMAnaSummary*)( evt -> GetPMAna(ibrec) );
      wsummary -> AddPMAna(pmanasum);
    }
