  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	while(fFindTimeClster(allhit, hitcls, fcTime)){
	  ...
	  for(int i=0;i<alltrack.size();i++){
	    ...
	    for(int NHIT=0;NHIT<alltrack[i].hitid.size();NHIT++){
	      inghitsum   = (IngridHitSummary*) (evt -> GetIngridModHit(alltrack[i].hitid[NHIT], mod, cyc) );
	      inghitsum -> isohit   = alltrack[i].isohit[NHIT];
	      inghitsum -> gocosmic = true;
	      recon -> AddIngridHit(inghitsum);
	    }
	  ...
	  //if(disp&&simver->inttype==11&&alltrack.size()==4&&mod==16){
	  //if(disp&&simver->inttype==11&&alltrack.size()==4&&mod==16){
	  //if(disp&&simver->inttype==1){

	 // if(disp&&simver->inttype==31){
	   if(disp&&simver->inttype==2 &&alltrack.size()==3/*&&mod==16*/){
	    for(int i=0;i<alltrack.size();i++){
	      cout<<(i+1);
