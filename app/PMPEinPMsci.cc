...
#include "TApplication.h"
#include "PMrecon.hxx"
//#include "PMPEinPM.hxx"
#include "PMPEinPMsci.hxx"
...
int main(int argc,char *argv[]){
  ...
  wtree->Branch("veto",&veto,"veto/O");
  wtree->Branch("edge",&edge,"edge/O");

  wtree->Branch("pe",&pe,"pe/F");
  ...
  for(int ievt=Nini; ievt<nevt; ievt++){
	...
	veto = pmtrack[i].vetowtracking;
	edge = pmtrack[i].edgewtracking;
	
	for(int t=0;t<pmtrack[i].hitpe.size();t++){
	  pe=pmtrack[i].hitpe[t];
	  wtree -> Fill();
	}
      }
