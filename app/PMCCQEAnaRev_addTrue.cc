...
int main(int argc,char *argv[]){
    ...
    //get the h1 tree for kinematics
    ...
    int Ntrack,Ningtrack,inttype,mufc,pfc,mupdg,ppdg,modecp;//newadd
    float reweight[NWEIGHTS];
    float range,prange;
    float muscirange, muironrange;
    float pscirange, pironrange;
    int pene,ppene;
    ...
    mu[2]=0;p[2]=0;
    mu[3]=-1;p[3]=-1;
    nu[1]=0;nu[2]=-tan(3.65/180*3.141592);nu[3]=1;
    ...
    float muenergy_true2, mumomentum_true2;//GeV
    float penergy_true, pmomentum_true;//GeV
    float Qsq_true, xbjorken_true;
    float Qsq_true2, xbjorken_true2;
    float tmomtrans_true;
    float tmomtrans_true2;
    //float tmomtrans_approx;
    ...
    wtree->Branch("muenergy_true2",&muenergy_true2,"muenergy_true2/F");
    wtree->Branch("mumomentum_true2",&mumomentum_true2,"mumomentum_true2/F");
    
    wtree->Branch("nuE2",&nuE2,"nuE2/F");
    ...
    wtree->Branch("Qsq_true2",&Qsq_true2,"Qsq_true2/F");
    wtree->Branch("xbjorken_true2",&xbjorken_true2,"xbjorken_true2/F");
    wtree->Branch("tmomtrans_true2",&tmomtrans_true2,"tmomtrans_true2/F");
    
    //wtree->Branch("tmomtrans_approx",&tmomtrans_approx,"tmomtrans_approx/F");
    ...
    wtree->Branch("mode",&modecp,"mode/I");//newadd
   ...
   wtree->Branch("npioncount",&npioncount,"npioncount/I");
    ...
    for(int ievt=minevt; ievt<maxevt; ievt++){
        ...
        mumomentum_true = sqrt(pow(muang_tarray[0],2)+pow(muang_tarray[1],2)+pow(muang_tarray[2],2));
        
        penergy_true = pang_tarray[3];
        pmomentum_true = sqrt(pow(pang_tarray[0],2)+pow(pang_tarray[1],2)+pow(pang_tarray[2],2));
        ...
        if(mode==0){//what the mode==0 for?
            ...
            nuE2 = Enu;//add2test
            muenergy_true2 = sqrt(Pmu[0]*Pmu[0]+Pmu[1]*Pmu[1]+Pmu[2]*Pmu[2]+Mmu*Mmu);
            mumomentum_true2 = sqrt(Pmu[0]*Pmu[0]+Pmu[1]*Pmu[1]+Pmu[2]*Pmu[2]);
            Qsq_true2 = (Pmu[0]-Pnu[0])*(Pmu[0]-Pnu[0]) +   (Pmu[1]-Pnu[1])*(Pmu[1]-Pnu[1])
                    +   (Pmu[2]-Pnu[2])*(Pmu[2]-Pnu[2]) -   (muenergy_true2-Enu)*(muenergy_true2-Enu);
            
            //cout<<"Check Mu Energy consistent "<<muang_tarray[3]<<" fromh1 "<<Emu<<endl;
            if(muang_tarray[2]!=-1) {
                Qsq_true = - pow(muang_tarray[0]-Pnu[0],2) - pow(muang_tarray[1]-Pnu[1],2) - pow(muang_tarray[2]-Pnu[2],2) + pow(muang_tarray[3]-Enu,2);
            }
            else Qsq_true = 0;
            
            //t transfer
            if (pang_tarray[2]!=-1) {
                tmomtrans_true = - pow(muang_tarray[0]+pang_tarray[0]-Pnu[0],2) - pow(muang_tarray[1]+pang_tarray[1]-Pnu[1],2) - pow(muang_tarray[2]+pang_tarray[2]-Pnu[2],2) + pow(muang_tarray[3]+pang_tarray[3]-Enu,2);
            }
            else tmomtrans_true = 0;
            
            
            //bjorken
            if((Enu - muang_tarray[3])!=0) xbjorken_true = Qsq_true/(2*(Enu - muang_tarray[3])*(0.939565379+0.938272046)/2);
            else xbjorken_true = 0;
        ...
        for(int i=0; i<npmana; i++){
            ...
            muscirange = 2 * 1.032 / 7.86 /1.43*1.77 * pmana->sci_range[0];
            muironrange = (6.5 + 2 * 1.032 / 7.86 /1.43*1.77) * pmana->iron_range[0];
            ...
            if(Ntrack>1){
                ...
                pscirange = 2 * 1.032 / 7.86 /1.43*1.77 * pmana->sci_range[1];
                pironrange = (6.5 + 2 * 1.032 / 7.86 /1.43*1.77) * pmana->iron_range[1];
                if(pmana->iron_pene[1]>9)
                    ppene = 9;
                else
            ...
            if(evt->NIngridSimVertexes()>0){
                ...
                nuE      = simver -> nuE;
                //cout<<"Check Energy consistent "<<nuE<<" fromh1 "<<Enu<<endl;
                //cout<<"Check Energy consistent 0 "<<nuE*0<<" fromh1 "<<Pnu[0]<<endl;
                //cout<<"Check Energy consistent 1 "<<nuE*(-0.064)<<" fromh1 "<<Pnu[1]<<endl;
                //cout<<"Check Energy consistent 2 "<<nuE*(0.998)<<" fromh1 "<<Pnu[2]<<endl;
                xnu      = simver -> xnu;
                ynu      = simver -> ynu;
                znu      = simver -> znu;
                ...
                //tmomtrans_true =pow((muang_tarray[3]+pang_tarray[3]-muang_tarray[2]-pang_tarray[2]),2)+pow((muang_tarray[1]+pang_tarray[1]),2)+pow((muang_tarray[2]+pang_tarray[2]),2);
            }
            else{
                norm=1;
                totcrsne=1;
