...
int main(int argc,char *argv[]){
  ...
  ifstream pevarifile("/home/cvson/cc1picoh/frkikawa/PM_Ana/app/pevar_ingrid.txt");
  ifstream pevarsfile("/home/cvson/cc1picoh/frkikawa/PM_Ana/app/pevar_scibar.txt");
  for(int iang=0;iang<24;iang++){
    pevarifile>>pevari[iang];
    pevarsfile>>pevars[iang];
  }
  
	cout<<"Testing "<<pevari[5]<<" scibar "<<pevars[5]<<endl;
...
void GetNonRecHits(IngridEventSummary* evt, int cyc){
  ...
  for(int ihit=0; ihit<ninghit; ihit++){
    ...
    ch   = inghitsum->ch;
    pln  = inghitsum->pln;
    pe   = PE(inghitsum->pe,inghitsum->lope,16,view,pln,ch,-1);
  ...
  for(int mod=0; mod<7; mod++){
    ...
    for(int ihit=0; ihit<ninghit; ihit++){
      ...
      ch   = inghitsum->ch;
      pln  = inghitsum->pln;
      pe   = PE(inghitsum->pe,inghitsum->lope,mod,view,pln,ch,-1);
