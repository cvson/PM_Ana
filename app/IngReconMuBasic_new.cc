...
int main(int argc,char *argv[]){
  ...
  bool renamef =false;
  bool cosmic=false;
  while ((c = getopt(argc, argv, "r:s:f:b:o:c")) != -1) {
    switch(c){
    case 'r':
	...
	*/
	...
	if( frecontrack -> ReconTrack(1) ){
	  ...
	  if( !isVeto(endpln) && endpln <  10 &&
	    ...
	    basicrecon -> muE     = 
	      1.0 * ( 1.0 * MIPdepE * basicrecon -> penIron * IronDensity * IronThick +
		      MIPdepE     * penScinti * 2 ) / TMath::Cos( basicrecon -> angle * TMath::Pi()/180 );
