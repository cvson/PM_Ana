...
int main(int argc,char *argv[]){
  ...
  for( int ievt = 0; ievt < nevt; ievt++  ){
    ...
    */   
    ...
    memset(hitcycmod,false,sizeof(hitcycmod));

    for( int ibrec=0; ibrec < evt -> NIngridBasicRecons(); ibrec++ ){
      IngridBasicReconSummary* ingbasrec =  (IngridBasicReconSummary*)( evt -> GetBasicRecon(ibrec) );
      //if(!ingbasrec->ontime)continue;
      //if(!(ingbasrec->hastrk))continue;
      //if(!(ingbasrec->matchtrk))continue;
      CYC=ingbasrec->hitcyc;
      MOD=ingbasrec->hitmod;
      if(MOD<14&CYC<21)hitcycmod[CYC][MOD]=true;
    }


    for( int mod=0; mod<14; mod++ ){   //### Module Loop
      for( int cyc=0; cyc<21; cyc++ ){  //### Cycle Loop
	if(!hitcycmod[cyc][mod])continue;
